...
def normalize_path(path, base_dir=None):
    ...
    # Join with base_dir if provided
...
def crop_and_save_image(row, image_dir, output_dir, target_size=(224, 224)):
    ...
    try:
        ...
        base_name = os.path.splitext(os.path.basename(img_relative_path))[0]  # Remove .jpg if exists
...
def crop_and_save_images(df, image_dir, output_dir, target_size=(224, 224)):
    ...
    for result in results:
        if result is not None:
            ...
            # Use the original image_name from the FINAL_MERGED_CSV
            original_image_name = image_name  # This is already the original name from the CSV
...
def create_landmark_heatmap(image_size, landmarks, sigma=5):
    """Generate a heatmap using NumPy without explicit loops."""
    ...
    # Convert landmark list to NumPy array for vectorized calculations
...
def process_heatmap_row(row, image_sizes, output_dir):
    ...
    # Extract landmark coordinates efficiently
    ...
    # Save heatmap efficiently - remove the .jpg extension before adding .npy
    base_name = os.path.splitext(os.path.basename(img_relative_path))[0] # Remove extra .jpg if present
...
def generate_and_save_heatmaps(df, image_sizes, output_dir):
    """Generate and save heatmaps in parallel."""
    os.makedirs(output_dir, exist_ok=True)
...
if __name__ == "__main__":
    ...
    if os.path.exists(FINAL_MERGED_CSV):
        ...
        # Step 1: Crop and save images (COMMENTED OUT)
        # logger.info("Processing images...")
        # cropped_image_sizes = crop_and_save_images(merged_df, RAW_DATA_DIR, os.path.join(PROCESSED_DATA_DIR, "cropped_images"))
        
        # Step 2: Generate heatmaps (COMMENTED OUT)
        # logger.info("Generating heatmaps...")
        # metadata_csv_path = os.path.join(PROCESSED_DATA_DIR, "metadata.csv")
        # cropped_images_dir = os.path.join(PROCESSED_DATA_DIR, "cropped_images")

        # Step 3: Verify cropped images and generate metadata
        logger.info("Generating metadata...")
        cropped_images_dir = os.path.join(PROCESSED_DATA_DIR, "cropped_images")
        metadata_csv_path = os.path.join(PROCESSED_DATA_DIR, "metadata.csv")

        # Get list of existing cropped images
        metadata = []
        for root, _, files in os.walk(cropped_images_dir):
            for file in files:
                if file.endswith(".jpg"):
                    # Get the full relative path (including subfolders)
                    img_relative_path = os.path.relpath(os.path.join(root, file), PROCESSED_DATA_DIR)
                    
                    # Extract the base name (without .jpg)
        ...
        # Step 4: Generate heatmaps (COMMENTED OUT)
        # generate_and_save_heatmaps(merged_df, cropped_image_sizes, os.path.join(PROCESSED_DATA_DIR, "heatmaps"))
