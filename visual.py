...
def mixup_data(x, y, alpha=0.05):
    """Mixup with alpha=0.05 for milder label blending."""
    if alpha > 0:
...
# ----------------------------
# FocalLoss for category classification
# ----------------------------
...
# ----------------------------
# (Optional) FocalLoss for multi-label attributes
# ----------------------------
...
def train_epoch(model, train_loader, criterion, optimizer, device, epoch, scaler, total_epochs):
    ...
    for batch_idx, batch in progress_bar:
        ...
        # --- Choose augmentation: Mixup (20%), CutMix (20%), or none ---
        ...
        # If using OneCycleLR, uncomment scheduler.step() here and remove epoch-level step.
        # scheduler.step()
        scaler.update()
...
def validate(model, val_loader, criterion, device, epoch):
    ...
    # Optional: Log classification report (per-class metrics)
    # from sklearn.metrics import classification_report
    # preds = outputs['category_logits'].argmax(dim=1).cpu().numpy()
    # trues = targets['category_labels'].cpu().numpy()
    # report = classification_report(trues, preds, labels=list(range(num_categories)), output_dict=True)
...
def main(args):
    ...
    # ---- Adjust Loss Weighting and override losses with FocalLoss ----
    criterion = FashionMultiTaskLoss(
        attribute_weight=0.4,
        aux_weight=0.1,
    ).to(device)
    criterion.category_loss_fn = FocalLoss(alpha=0.25, gamma=2.0)
    # Optionally override attribute loss:
    # criterion.attribute_loss_fn = FocalLossMultiLabel(alpha=0.25, gamma=2.0)
    ...
    # ---- Option 1: LambdaLR scheduler (current) ----
    warmup_epochs = 5
    ...
    # ---- Option 2: OneCycleLR scheduler (uncomment to use) ----
    # from torch.optim.lr_scheduler import OneCycleLR
    # steps_per_epoch = len(train_loader)
    # scheduler = OneCycleLR(optimizer, max_lr=args.learning_rate, epochs=args.epochs, steps_per_epoch=steps_per_epoch)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Optionally, you can call model.freeze_backbone(False) at a specific epoch as before.
        if epoch == 5:
            if hasattr(model, 'module'):
        ...
        res_train = train_epoch(model, train_loader, criterion, optimizer, device, epoch, scaler, args.epochs)
