...
from models.modelerror_upd import FashionHybridModel, FashionMultiTaskLoss, compute_f1_multilabel, compute_f1_multiclass
from utils.data_collab_upd import FashionDataset
...
def validate(model, val_loader, criterion, device, epoch):
    ...
    logger.info(f"Validation F1 Scores: "
                f"Category={avg_f1_category:.4f}, "
                f"Type={avg_f1_category_type:.4f}, "
                f"Attributes={avg_f1_attributes:.4f}")
    logger.info(f"Validation Accuracy: "
                f"Category={avg_accuracy_category:.4f}, "
                f"Type={avg_accuracy_category_type:.4f}, "
                f"Attributes={avg_accuracy_attributes:.4f}")
...
def main(args):
    ...
    # Optional: Subset the validation set
    ...
    # (Here we've removed label smoothing in the model script—see that part)
    criterion = FashionMultiTaskLoss(
        attribute_weight=0.7,
        # You can tune other weights if needed
        # label_smoothing removed from the internal CrossEntropy initialization
    ).to(device)
