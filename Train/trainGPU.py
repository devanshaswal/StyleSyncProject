...
def train_epoch(model, train_loader, criterion, optimizer, device, epoch):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        # logger.info(f"Image tensor device: {images.device}")
...
def main(args):
    ...
    # Force CUDA if available (ignore --no-cuda flag)
    ...
    # Explicitly move model to GPU and use DataParallel if multiple GPUs are available
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--no-cuda', action='store_true', help='This flag is ignored in this version')
    
    # Define args for testing but force GPU usage regardless of --no-cuda flag
