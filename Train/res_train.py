...
from sklearn.metrics import (accuracy_score,  # Used for per-attribute F1 below
                             f1_score)
from torch.cuda.amp import GradScaler, autocast
# Optionally comment out cosine annealing if using OneCycleLR
...
# ----------------------------
# Helper function: Unfreeze Backbone Layers Gradually
# ----------------------------
...
def train_epoch(model, train_loader, criterion, optimizer, device, epoch, scaler, total_epochs, scheduler):
    ...
    for batch_idx, batch in progress_bar:
        ...
        # --- Choose augmentation: Mixup (10%), CutMix (10%), or none ---
        ...
        # Direct backward pass without gradient accumulation:
...
def validate(model, val_loader, criterion, device, epoch):
    ...
    # New: To accumulate per-batch predictions for per-attribute F1
    all_attr_targets = []
    all_attr_preds = []
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            # Accumulate predictions and targets for per-attribute F1
    ...
    # New: Calculate per-attribute F1 scores using sklearn.metrics.f1_score with average=None
    ...
    # Identify the worst 5 performing attributes
    worst_5_indices = np.argsort(per_attribute_f1)[:5]
    # Retrieve attribute names from your dataset; adjust as needed if using Subset objects
...
def main(args):
    ...
    # Compute attribute frequencies and adjust class weights
    ...
    # Remove compatibility head from the model if needed (see model definition)
    ...
    # Adjust Loss Weighting and override losses with FocalLoss using computed attribute class weights
    criterion = FashionMultiTaskLoss(
        category_weight=1.0,
        attribute_weight=1.5,
        aux_weight=0.05,
        compatibility_weight=0.0,
        alpha_attributes=alpha_attributes  # Pass computed attribute class weights
    ).to(device)
    criterion.category_loss_fn = FocalLoss(alpha=0.25, gamma=2.0)
    # Note: Do not override attribute_loss_fn as it now uses computed alpha_attributes
    ...
    # Early stopping based on attribute micro F1
    best_attr_f1 = 0.0
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        writer.add_scalar('F1/val_attributes', val_f1_attr, epoch)  # Tracking micro F1 for attributes
        ...
        # Update early stopping based on attribute F1 (micro F1)
