    ...
    for batch_idx, batch in progress_bar:
        ...
        try:
            with torch.amp.autocast("cuda"):
                ...
                # Compute standard category loss (no mixup)
                category_loss = criterion.category_loss_fn(outputs['category_logits'], category_labels)
                ...
                image_aux_loss = criterion.category_loss_fn(outputs['image_aux_logits'], category_labels)
                ...
                loss = (criterion.category_weight * category_loss +
        ...
        # Compute metrics
        f1_category = compute_f1_multiclass(outputs['category_probs'], category_labels)
        accuracy_category = (outputs['category_logits'].argmax(dim=1) == category_labels).float().mean()
...
def validate(model, val_loader, criterion, device, epoch):
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            category_labels = targets['category_labels']

            # Update per-class performance
            for pred, true in zip(predicted_category, category_labels):
                cls = true.item()
    ...
    problematic_classes = []
    for cls, stats in sorted(class_performance.items()):
        if stats['total'] > 0:
            accuracy = stats['correct'] / stats['total']
            logger.info(f"Class {cls}: {accuracy:.2%} ({stats['correct']}/{stats['total']})")
            if accuracy < 0.3:
                problematic_classes.append((cls, accuracy))

    if problematic_classes:
        print("\n--- PROBLEMATIC CLASSES ---")
        for cls, acc in problematic_classes:
            print(f"Class {cls}: Very Low Accuracy {acc:.2%}")
...
def main(args):
    ...
    # Disable BatchNorm running statistics
    ...
    # Initialize custom multi-task loss
    criterion = FashionMultiTaskLoss(attribute_weight=1.0).to(device)
    ...
    start_epoch = 13  # Default starting epoch
    ...
    for epoch in range(args.epochs):
        # Unfreeze backbone at epoch 5 if needed
        if epoch == 5:
            if hasattr(model, 'module'):
        ...
        (train_loss, train_f1_category, train_f1_category_type, train_f1_attributes,
         train_accuracy_category, train_accuracy_category_type, train_accuracy_attributes, _) = results_train

        results_val = validate(model, val_loader, criterion, device, epoch)
        (val_loss, val_category_acc, val_type_acc, val_attributes_acc,
         val_f1_category, val_f1_category_type, val_f1_attributes,
         val_accuracy_category, val_accuracy_category_type, val_accuracy_attributes, _) = results_val

        print(f" Validation Epoch {epoch} | Loss: {val_loss:.4f}")
        print(f"  Validation F1 Scores - Category: {val_f1_category:.4f}, "
              f"Type: {val_f1_category_type:.4f}, Attributes: {val_f1_attributes:.4f}")
        print(f" Validation Accuracy - Category: {val_accuracy_category:.4f}, "
              f"Type: {val_accuracy_category_type:.4f}, Attributes: {val_accuracy_attributes:.4f}")
