...
# ----------------------------
# FocalLoss for multi-label attributes with per-class weighting support
# ----------------------------
...
class FashionHybridModel(nn.Module):
    def __init__(self, num_categories=46, num_category_types=3, num_attributes=50,
        ...
        # New adapter to reduce channel depth and aggregate spatial features
        ...
        # Updated heatmap encoder with increased output dimension
        ...
        self.heatmap_feature_dim = 128  # Updated output dimension

        # Updated fusion layer: fuse image features (512) and heatmap features (128)
        total_feature_dim = 512 + 128  # 640 total
        ...
        self.attribute_predictor = nn.Sequential(
            nn.Linear(embed_dim, 1024),  # Increased capacity
            nn.BatchNorm1d(1024),
            nn.ReLU(),
    ...
    def forward(self, image, heatmap):
        # Extract image features from ResNet's layer3 and adapt them
