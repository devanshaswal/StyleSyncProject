...
def train_epoch(model, train_loader, criterion, optimizer, device, epoch, scaler, total_epochs):
    ...
    for batch_idx, batch in progress_bar:
        ...
        # --- Mixup with 20% probability, alpha=0.05 ---
        ...
        try:
            with torch.amp.autocast('cuda'):
                ...
                # Compute category loss with mixup
...
def main(args):
    ...
    # Data Augment - less aggressive
    train_transform = transforms.Compose([
        transforms.RandomResizedCrop(224, scale=(0.9, 1.0)),  # narrower scale
        transforms.RandomHorizontalFlip(),
        transforms.RandomRotation(15),                       # was 30
        transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.1),  # was 0.5
    ...
    assert 45 in category_counts, "Category 45 missing"
    ...
    # Possibly freeze backbone initially
    if hasattr(model, 'module'):
        model.module.freeze_backbone(freeze=False)
    else:
        model.freeze_backbone(freeze=False)

    # Adjust loss weighting: attribute=0.4, aux=0.1
    criterion = FashionMultiTaskLoss(
        attribute_weight=0.4,  # down from 0.7
        aux_weight=0.1,        # down from 0.3
    ).to(device)
...
class Args:
    def __init__(self):
        ...
        self.batch_size = 512      # Consider 128 if you run into VRAM issues
        self.epochs = 20
